<?php
/*
Plugin Name: Organics Utilities
Plugin URI: http://axiomthemes.net
...
if (!function_exists('organics_require_shortcode')) {	
...
/* Twitter API
------------------------------------------------------ */
if (!function_exists('organics_twitter_acquire_data')) {
	function organics_twitter_acquire_data($cfg) {
		...
		if (!$data) {
			...
			$code = $tmhOAuth->user_request(array(
				'url' => $tmhOAuth->url(organics_twitter_mode_url($cfg['mode']))
			));
			if ($code == 200) {
				$data = json_decode($tmhOAuth->response['response'], true);
				if (isset($data['status'])) {
					$code = $tmhOAuth->user_request(array(
						'url' => $tmhOAuth->url(organics_twitter_mode_url($cfg['oembed'])),
						'params' => array(
...
if (!function_exists('organics_twitter_mode_url')) {
	function organics_twitter_mode_url($mode) {
		$url = '/1.1/statuses/';
...
// Compile less-files
if (!function_exists('organics_less_compiler')) {	
	function organics_less_compiler($list, $opt) {
		...
		foreach($list as $file) {
			...
			try {
				...
				if ($opt['compiler'] == 'lessc') {
					$parser = new lessc;
					//$parser->registerFunction("replace", "organics_less_func_replace");
				...
				if ($css) {
					if ($opt['map']=='no') {
						...
						if ($file == get_template_directory(). '/style.less') {
							...
							$css =	"/*"
									. "\n"
									. __('Attention! Do not modify this .css-file!', 'axiomthemes') 
									. "\n"
									. __('Please, make all necessary changes in the corresponding .less-file!', 'axiomthemes')
									. "\n"
									. "*/"
									. "\n"
									. '@charset "utf-8";'
...
// LESS function
/*
if (!function_exists('organics_less_func_replace')) {	
	function organics_less_func_replace($arg) {
    	return $arg;
	}
}
*/
