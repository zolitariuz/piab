jQuery(document).ready(function () {
	...
	jQuery('#frontend_editor_icon_edit').click(function (e) {
		...
		jQuery('#frontend_editor_button_cancel').val(ORGANICS_GLOBALS['strings']['editor_caption_cancel']);
	...
	jQuery('#frontend_editor_button_cancel').click(function (e) {
		"use strict";
		if (jQuery(this).val() == ORGANICS_GLOBALS['strings']['editor_caption_close'])
	...
	jQuery('#frontend_editor_button_save').click(function (e) {
		...
		var data = {
			...
			nonce: ORGANICS_GLOBALS['ajax_nonce_editor'],
		...
		jQuery.post(ORGANICS_GLOBALS['ajax_url'], data, function(response) {
			...
			if (rez.error == '') {
				organics_message_success('', ORGANICS_GLOBALS['strings']['editor_save_success']);
				jQuery('#frontend_editor_button_cancel').val(ORGANICS_GLOBALS['strings']['editor_caption_close']);
			} else {
				organics_message_warning(rez.error, ORGANICS_GLOBALS['strings']['editor_save_error']);
			}
		});
		e.preventDefault();
		return false;
	...
	jQuery('#frontend_editor_icon_delete').click(function (e) {
		"use strict";
		organics_message_confirm(ORGANICS_GLOBALS['strings']['editor_delete_post'], ORGANICS_GLOBALS['strings']['editor_delete_post_header'], function(btn) {
			"use strict";
			if (btn != 1) return;
			var data = {
			...
			jQuery.post(ORGANICS_GLOBALS['ajax_url'], data, function(response) {
				...
				var rez = JSON.parse(response);
				if (rez.error == '') {
					organics_message_success('', ORGANICS_GLOBALS['strings']['editor_delete_success']);
					setTimeout(function() { 
						window.location.href = ORGANICS_GLOBALS['site_url'];
						}, 1000);
				} else {
					organics_message_warning(rez.error, ORGANICS_GLOBALS['strings']['editor_delete_error']);
				}
			});
