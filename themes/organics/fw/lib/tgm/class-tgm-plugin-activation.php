    ...
    class TGM_Plugin_Activation {
        ...
        public function __construct() {
            ...
            $this->strings = array(
                ...
                'installing'                     => esc_html__( 'Installing Plugin: %s', 'organics' ),
                'oops'                           => esc_html__( 'Something went wrong.', 'organics' ),
                ...
                'notice_can_install_required'    => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'organics' ),
                'notice_can_install_recommended' => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'organics' ),
                'notice_cannot_install'          => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'organics' ),
                'notice_can_activate_required'   => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'organics' ),
                'notice_can_activate_recommended'=> _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'organics' ),
                'notice_cannot_activate'         => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'organics' ),
                'notice_ask_to_update'           => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'organics' ),
                'notice_cannot_update'           => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'organics' ),
                'install_link'                   => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'organics' ),
                'activate_link'                  => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'organics' ),
                'return'                         => esc_html__( 'Return to Required Plugins Installer', 'organics' ),
                'dashboard'                      => esc_html__( 'Return to the dashboard', 'organics' ),
                'plugin_activated'               => esc_html__( 'Plugin activated successfully.', 'organics' ),
                'activated_successfully'         => esc_html__( 'The following plugin was activated successfully:', 'organics' ),
                'complete'                       => esc_html__( 'All plugins installed and activated successfully. %1$s', 'organics' ),
                'dismiss'                        => esc_html__( 'Dismiss this notice', 'organics' ),
            );
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' == $_GET['tgmpa-install'] ) ) {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . esc_url( network_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'organics' ) . '">' . __( 'Return to the Dashboard', 'organics' ) . '</a>' ) . '</p>';
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'organics' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), network_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'organics' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), network_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'organics' ) . '</a>'  : '';
                ...
                if ( ! empty( $this->strings['nag_type'] ) ) {
                    add_settings_error( 'organics', 'organics', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
                } else {
                    add_settings_error( 'organics', 'organics', $rendered, $nag_class );
                }
            }

            // Admin options pages already output settings_errors, so this is to avoid duplication.
            if ( 'options-general' !== $current_screen->parent_base ) {
                settings_errors( 'organics' );
            }
    ...
    class TGMPA_List_Table extends WP_List_Table {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = esc_html__( 'Private Repository', 'organics' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = esc_html__( 'Pre-Packaged', 'organics' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = esc_html__( 'WordPress Repository', 'organics' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? esc_html__( 'Required', 'organics' ) : esc_html__( 'Recommended', 'organics' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', esc_html__( 'Not Installed', 'organics' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', esc_html__( 'Installed But Not Activated', 'organics' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Install', 'organics' ) . ' %2$s">' . esc_html__( 'Install', 'organics' ) . '</a>',
                        esc_url(
                            wp_nonce_url(
                                add_query_arg(
                                    array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Activate', 'organics' ) . ' %2$s">' . esc_html__( 'Activate', 'organics' ) . '</a>',
                        esc_url(
                            add_query_arg(
                                array(
                                    'page'                 => urlencode( TGM_Plugin_Activation::$instance->menu ),
        ...
        public function column_cb( $item ) {
            ...
            if ( esc_html__( 'Private Repository', 'organics' ) === $item['source'] ) {
                // Escape external URLs
                $plugin_url = esc_url( $plugin_url );
            } elseif ( esc_html__( 'Pre-Packaged', 'organics' ) === $item['source'] ) {
        ...
        public function no_items() {
            ...
            printf( wp_kses( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'organics' ), 
					array('a'=>array('href'=>array(), 'title'=>array())) ), network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => esc_html__( 'Plugin', 'organics' ),
                'source' => esc_html__( 'Source', 'organics' ),
                'type'   => esc_html__( 'Type', 'organics' ),
                'status' => esc_html__( 'Status', 'organics' )
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'tgmpa-bulk-install'  => esc_html__( 'Install', 'organics' ),
                'tgmpa-bulk-activate' => esc_html__( 'Activate', 'organics' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s.</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'organics' ), $imploded );
                }
            ...
            class TGM_Bulk_Installer extends WP_Upgrader {
                ...
                public function run( $options ) {
                    ...
                    public function install_strings() {
                        ...
                        $this->strings['no_package']          = esc_html__( 'Install package not available.', 'organics' );
                        $this->strings['downloading_package'] = wp_kses( __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'organics' ), array('span'=>array('class'=>array())) );
                        $this->strings['unpack_package']      = esc_html__( 'Unpacking the package&#8230;', 'organics' );
                        $this->strings['installing_package']  = esc_html__( 'Installing the plugin&#8230;', 'organics' );
                        $this->strings['process_failed']      = esc_html__( 'Plugin install failed.', 'organics' );
                        $this->strings['process_success']     = esc_html__( 'Plugin installed successfully.', 'organics' );
                    ...
                    public function activate_strings() {
                        ...
                        $this->strings['activation_failed']  = esc_html__( 'Plugin activation failed.', 'organics' );
                        $this->strings['activation_success'] = esc_html__( 'Plugin activated successfully.', 'organics' );
            ...
            if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
                ...
                class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                    ...
                    public function add_strings() {
                        ...
                        if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                            $this->upgrader->strings['skin_upgrade_start']        = esc_html__( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'organics' );
                            $this->upgrader->strings['skin_update_successful']    = esc_html__( '%1$s installed and activated successfully.', 'organics' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . esc_html__( 'Show Details', 'organics' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', 'organics' ) . '</span>.</a>';
                            $this->upgrader->strings['skin_upgrade_end']          = esc_html__( 'All installations and activations have been completed.', 'organics' );
                            $this->upgrader->strings['skin_before_update_header'] = esc_html__( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'organics' );
                        }
                        // Default installation strings.
                        else {
                            $this->upgrader->strings['skin_upgrade_start']        = esc_html__( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'organics' );
                            $this->upgrader->strings['skin_update_failed_error']  = esc_html__( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'organics' );
                            $this->upgrader->strings['skin_update_failed']        = esc_html__( 'The installation of %1$s failed.', 'organics' );
                            $this->upgrader->strings['skin_update_successful']    = esc_html__( '%1$s installed successfully.', 'organics' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . esc_html__( 'Show Details', 'organics' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', 'organics' ) . '</span>.</a>';
                            $this->upgrader->strings['skin_upgrade_end']          = esc_html__( 'All installations have been completed.', 'organics' );
                            $this->upgrader->strings['skin_before_update_header'] = esc_html__( 'Installing Plugin %1$s (%2$d/%3$d)', 'organics' );
                        }
                    ...
                    public function bulk_footer() {
                        ...
                        if ( empty( $complete ) ) {
                            echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . esc_url( network_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'organics' ) . '">' . esc_html__( 'Return to the Dashboard', 'organics' ) . '</a>' ) . '</p>';
                            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                        }
